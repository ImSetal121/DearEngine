...
SDL_GPUShader* loadSDLGPUShader(const char* filename, SDL_Storage* storage, SDL_GPUShaderStage stage, SDL_GPUShaderFormat format) {
    Uint64 file_size;
...
GPUShaderBundle createSDLGPUShaderBundle() {
    ...
    if (cwd) {
        ...
        SDL_LogWarn(SDL_LOG_CATEGORY_SYSTEM, "Could not get current directory: %s", SDL_GetError());
    }
    ...
    const char* backend = SDL_GetGPUDeviceDriver(gDevice);
    const bool useMetal = backend && (SDL_strcasecmp(backend, "metal") == 0);

    if (useMetal) {
        bundle.vertex   = loadSDLGPUShader("vert.msl", storage, SDL_GPU_SHADERSTAGE_VERTEX,   SDL_GPU_SHADERFORMAT_MSL);
        bundle.fragment = loadSDLGPUShader("frag.msl", storage, SDL_GPU_SHADERSTAGE_FRAGMENT, SDL_GPU_SHADERFORMAT_MSL);
    } else {
        bundle.vertex   = loadSDLGPUShader("vert.spv", storage, SDL_GPU_SHADERSTAGE_VERTEX,   SDL_GPU_SHADERFORMAT_SPIRV);
        bundle.fragment = loadSDLGPUShader("frag.spv", storage, SDL_GPU_SHADERSTAGE_FRAGMENT, SDL_GPU_SHADERFORMAT_SPIRV);
    }
