...
namespace {
    ...
    static void RecursionEditorStartEntity(DE::Entity* entity, void* appstate) {
    if (!entity) return;
    for (auto& kv : entity->components)
        kv.second->EditorStart(appstate);
    for (auto& child : entity->children)
        RecursionEditorStartEntity(child.get(), appstate);
}

    // 递归：对单个实体及其所有子实体的组件调用 EditorIterate
    static void RecursionEditorIterateEntity(DE::Entity* entity, void* appstate) {
    if (!entity) return;
    for (auto& kv : entity->components)
        kv.second->EditorIterate(appstate);
    for (auto& child : entity->children)
        RecursionEditorIterateEntity(child.get(), appstate);
}
    ...
    void Scene::EditorStartAllEntity(void *appstate) {
        for (auto& e : this->root) {
            RecursionEditorStartEntity(e.get(), appstate);
        }
    }

    void Scene::EditorIterateAllEntity(void *appstate) {
        for (auto& e : this->root) {
            RecursionEditorIterateEntity(e.get(), appstate);
        }
    }
} // namespace DE
